...
class TestExpandMoneyArgs:
    ...
    def test_exact(self):
        ...
        actual = _expand_money_args(
            ModelWithNonMoneyField(), [Q(money=Money(0, "USD"))]
        )
    ...
    def test_and(self):
        ...
        """
        actual = _expand_money_args(
            ModelWithNonMoneyField(), [Q(money=Money(0, "USD"), desc="foo")]
        )
        ...
        for child in arg.children:
            if isinstance(child, tuple):
                ...
                pytest.fail(
                    "There should only be two elements, a tuple and a Q - not a %s"
                    % child
                )
    ...
    def test_and_with_or(self):
        ...
        actual = _expand_money_args(
            ...
            [Q(money=Money(0, "USD"), desc="foo") | Q(desc="bar")],
        )
    ...
    def test_and_with_two_or(self):
        ...
        actual = _expand_money_args(
            ModelWithNonMoneyField(),
            [Q(Q(money=Money(0, "USD"), desc="foo") | Q(desc="eggs")) | Q(desc="bar")],
        )
        arg = actual[0]
            ...
            if isinstance(child, tuple):
                ...
                for subchild in child.children:
                    if isinstance(subchild, tuple):
                        ...
                        pytest.fail(
                            "There should only be two subchild elements, a tuple and a Q - not a %s"
                            % subsubchild
                        )
            else:
                pytest.fail(
                    "There should only be two child elements, a tuple and a Q - not a %s"
                    % child
                )
...
class TestKwargsExpand:
    @pytest.mark.parametrize(
        ...
        (
            (
                ...
                (
                    {"money": OldMoney(100, "USD")},
                    {"money": 100, "money_currency": "USD"},
                ),
                (
                    {"money": Money(100, "USD"), "desc": "test"},
                    {"money": 100, "money_currency": "USD", "desc": "test"},
                ),
            )
        ),
    )
    ...
    def test_simple_f_query(self):
        _, kwargs = _expand_money_kwargs(
            ModelWithNonMoneyField, kwargs={"money": F("money")}
        )
        assert isinstance(kwargs["money_currency"], F)
        assert kwargs["money_currency"].name == "money_currency"
