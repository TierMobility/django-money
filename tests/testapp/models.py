...
class ModelWithDefaultAsInt(models.Model):
    money = MoneyField(
        default=123, max_digits=10, decimal_places=2, default_currency="GHS"
    )
...
class ModelWithDefaultAsString(models.Model):
    money = MoneyField(
        default="123", max_digits=10, decimal_places=2, default_currency="PLN"
    )


class ModelWithDefaultAsFloat(models.Model):
    money = MoneyField(
        default=12.05, max_digits=10, decimal_places=2, default_currency="PLN"
    )


class ModelWithDefaultAsDecimal(models.Model):
    money = MoneyField(
        default=Decimal("0.01"), max_digits=10, decimal_places=2, default_currency="CHF"
    )
...
class NullMoneyFieldModel(models.Model):
    field = MoneyField(
        max_digits=10, decimal_places=2, null=True, default_currency="USD", blank=True
    )
...
class ModelIssue300(models.Model):
    ...
    price = MoneyField(
        max_digits=10, decimal_places=2, default_currency="EUR", default=Decimal("0.0")
    )
...
class PositiveValidatedMoneyModel(models.Model):
    ...
    money = MoneyField(
        max_digits=10, decimal_places=2, validators=[MinMoneyValidator(0)]
    )
