...
class TestVanillaMoneyField:
    ...
    def objects_setup(self, request):
        ...
        ModelWithTwoMoneyFields.objects.bulk_create(
            (
                ModelWithTwoMoneyFields(
                    amount1=Money(1, "USD"), amount2=Money(2, "USD")
                ),
                ModelWithTwoMoneyFields(
                    amount1=Money(2, "USD"), amount2=Money(0, "USD")
                ),
                ModelWithTwoMoneyFields(
                    amount1=Money(3, "USD"), amount2=Money(0, "USD")
                ),
                ModelWithTwoMoneyFields(
                    amount1=Money(4, "USD"), amount2=Money(0, "GHS")
                ),
                ModelWithTwoMoneyFields(
                    amount1=Money(5, "USD"), amount2=Money(5, "USD")
                ),
                ModelWithTwoMoneyFields(
                    amount1=Money(5, "EUR"), amount2=Money(5, "USD")
                ),
            )
        )
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            (
                ...
                | Q(amount1=Money(4, "USD"))
                | Q(amount2=Money(0, "GHS")),
                2,
            ),
            (
                Q(amount1=OldMoney(1, "USD"))
                | Q(amount1=OldMoney(4, "USD"))
                | Q(amount2=OldMoney(0, "GHS")),
                2,
            ),
            (
                Q(amount1=Money(1, "USD"))
                | Q(amount1=Money(5, "USD"))
                | Q(amount2=Money(0, "GHS")),
                3,
            ),
            (
                Q(amount1=Money(1, "USD"))
                | Q(amount1=Money(4, "USD"), amount2=Money(0, "GHS")),
                2,
            ),
            (
                Q(amount1=Money(1, "USD"))
                | Q(amount1__gt=Money(4, "USD"), amount2=Money(0, "GHS")),
                1,
            ),
            (
                Q(amount1=Money(1, "USD"))
                | Q(amount1__gte=Money(4, "USD"), amount2=Money(0, "GHS")),
                2,
            ),
        ),
    )
    ...
    @pytest.mark.skipif(
        VERSION[:2] == (1, 8), reason="Django 1.8 doesn't support __date lookup"
    )
    ...
    def test_in_lookup(self):
        assert (
            ModelWithTwoMoneyFields.objects.filter(
                amount1__in=(Money(1, "USD"), Money(5, "EUR"))
            ).count()
            == 2
        )
        assert (
            ModelWithTwoMoneyFields.objects.filter(
                ...
                amount1__in=(Money(1, "USD"), Money(3, "USD")),
            ).count()
            == 1
        )
        assert (
            ModelWithTwoMoneyFields.objects.exclude(
                amount1__in=(Money(1, "USD"), Money(5, "EUR"))
            ).count()
            == 4
        )
...
class TestGetOrCreate:
    @pytest.mark.parametrize(
        ...
        (
            ...
            (
                ModelWithSharedCurrency,
                "first",
                {"first": 10, "second": 15, "currency": "CZK"},
                "CZK",
            ),
        ),
    )
    def test_get_or_create_respects_currency(self, model, field_name, kwargs, currency):
        ...
        assert (
            str(field.currency) == currency
        ), "currency should be taken into account in get_or_create"
    ...
    @pytest.mark.parametrize(
        ...
        (
            (
                NullMoneyFieldModel,
                {"field": Money(100, "USD")},
                {"field": 100, "field_currency": "USD"},
            ),
            (
                ModelWithSharedCurrency,
                {"first": 10, "second": 15, "currency": "USD"},
                {"first": 10, "currency": "USD"},
            ),
        ),
    )
...
class TestFExpressions:
    ...
    def test_filtration(self, create_kwargs, filter_value, in_result):
        ...
        assert (
            instance in ModelWithVanillaMoneyField.objects.filter(**filter_value)
        ) is in_result
...
class TestExpressions:
    def test_conditional_update(self):
        ...
        assert ModelWithVanillaMoneyField.objects.get(integer=0).money == Money(
            10, "USD"
        )
        assert ModelWithVanillaMoneyField.objects.get(integer=1).money == Money(
            0, "USD"
        )

    @pytest.mark.skipif(
        VERSION[:2] == (1, 8), reason="Django 1.8 doesn't supports this"
    )
    ...
    @pytest.mark.parametrize(
        "value, expected",
        ((None, None), (10, Money(10, "USD")), (Money(10, "EUR"), Money(10, "EUR"))),
    )
    ...
    def test_expressions_for_non_money_fields(self):
        instance = ModelWithVanillaMoneyField.objects.create(
            money=Money(1, "USD"), integer=0
        )
        assert (
            ModelWithVanillaMoneyField.objects.get(money=F("integer") + 1) == instance
        )
        assert (
            ModelWithVanillaMoneyField.objects.get(Q(money=F("integer") + 1))
            == instance
        )
...
def test_find_models_related_to_money_models():
    ...
    ModelRelatedToModelWithMoney.objects.get(
        moneyModel__money__lt=Money("1000.0", "ZWN")
    )
...
class TestInheritance:
    ...
    def test_fields(self, model_class):
        ...
        instance = model_class.objects.create(
            money=first_value, second_field=second_value
        )
...
@pytest.mark.parametrize(
    "model_class",
    (AbstractModel, ModelWithNonMoneyField, InheritorModel, InheritedModel, ProxyModel),
)
...
@pytest.mark.skipif(
    VERSION[:2] != (1, 8),
    reason="Only Django 1.8 has `get_field_by_name` method of `Options`.",
)
def test_get_field_by_name():
    assert (
        BaseModel._meta.get_field_by_name("money")[0].__class__.__name__ == "MoneyField"
    )
    assert (
        BaseModel._meta.get_field_by_name("money_currency")[0].__class__.__name__
        == "CurrencyField"
    )
...
def test_deprecation():
    ...
    assert (
        ...
        == "'djmoney.models.fields.MoneyPatched' is deprecated. "
...
class TestSharedCurrency:
    @pytest.fixture
    def instance(self):
        return ModelWithSharedCurrency.objects.create(
            first=10, second=15, currency="USD"
        )
    ...
    def test_f_query(self, args, kwargs):
        instance = ModelWithSharedCurrency.objects.create(
            first=10, second=10, currency="USD"
        )
        assert instance in ModelWithSharedCurrency.objects.filter(*args, **kwargs)
