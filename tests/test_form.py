...
def test_no_deprecation_warning():
    ...
    with pytest.warns(None) as warning:
        MoneyField(
            max_digits=10, decimal_places=2, currency_choices=(("USD", "USD"),)
        ).formfield()
    assert not warning
...
class TestValidation:
    ...
    @pytest.mark.parametrize(
        "value",
        (Money(150, "EUR"), Money(200, "USD"), Money(50, "SEK"), Money(600, "NOK")),
    )
    def test_valid(self, value):
        assert ValidatedMoneyModelForm(
            data={"money_0": value.amount, "money_1": value.currency}
        ).is_valid()

    @pytest.mark.parametrize(
        "value",
        (Money(-0.01, "EUR"), Money(-1, "USD"), Money(-10, "NOK"), Money(-100, "SEK")),
    )
    ...
    def test_default_django_validator(self):
        ...
        assert not form.is_valid()
        assert form.errors == {
            "balance": [u"Ensure this value is greater than or equal to GB£100.00."]
        }


@pytest.mark.skipif(
    VERSION[:2] == (1, 8), reason="Django 1.8 doesn't have `disabled` keyword in fields"
)
