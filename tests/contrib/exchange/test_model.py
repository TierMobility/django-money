...
@pytest.mark.parametrize(
    "source, target", (("NOK", "ZAR"), ("ZAR", "NOK"), ("USD", "ZAR"), ("ZAR", "USD"))
)
@pytest.mark.usefixtures("default_openexchange_rates")
def test_unknown_currency_with_partially_exiting_currencies(source, target):
    with pytest.raises(
        MissingRate, match="Rate %s \\-\\> %s does not exist" % (source, target)
    ):
...
def test_cache():
    with patch(
        "djmoney.contrib.exchange.models._get_rate", wraps=_get_rate
    ) as original:
