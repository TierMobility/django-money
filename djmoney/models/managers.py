...
def _convert_in_lookup(model, field_name, options):
    ...
    for value in options:
        if isinstance(value, MONEY_CLASSES):
            ...
            option = {
                field.name: value,
                get_currency_field_name(field.name, field): F(
                    get_currency_field_name(value.name, target_field)
                ),
            }
        else:
            # amount__in=[1, 2, 3]
    ...
    for i, child in enumerate(arg.children):
        if isinstance(child, Q):
            ...
            if isinstance(value, MONEY_CLASSES):
                ...
                arg.children[i] = Q(
                    child, (currency_field_name, smart_unicode(value.currency))
                )
            if isinstance(field, MoneyField):
                if isinstance(value, (BaseExpression, F)):
                    ...
                    arg.children[i] = Q(
                        ...
                        (
                            currency_field_name,
                            F(get_currency_field_name(value.name, target_field)),
                        ),
                    )
...
def _expand_money_kwargs(model, args=(), kwargs=None, exclusions=()):
    ...
    for name, value in list(kwargs.items()):
        ...
        if isinstance(value, MONEY_CLASSES):
            ...
            if isinstance(field, MoneyField):
                if isinstance(value, (BaseExpression, F)) and not isinstance(
                    value, Case
                ):
                    clean_name = _get_clean_name(model, name)
                    if not isinstance(value, F):
